...
bool branch(QTR nums[count])
{
  int left[4]={-10,-5,2,10};
  int right[4]={10,2,-5,-10};
  int center1;
  int center2;
  int sum=0;
...
void PID(float velocity) {
    ...
    float errors[3][count] = {{-12,-9,-6,-3,0,3,6,9,12},
                              {-8,-6,-4,-2,0,2,4,6,8},
                              {-4,-3,-2,-1,0,1,2,3,4}};
    float left[3][count] = {{-12,-9,-6,-3,0,0,0,0,0},
                              {-8,-6,-4,-2,0,0,0,0,0},
                              {-4,-3,-2,-1,0,0,0,0,0}};
    float right[3][count] = {{0,0,0,0,0,3,6,9,12},
                              {0,0,0,0,0,2,4,6,8},
                              {0,0,0,0,0,1,2,3,4}};
    ...
    for(int i = 0; i < 3; ++i) {
      for(int j = 0; j < count; ++j) {
        test[i][j] = 100 - qtrs[i][j];
      }
    }

    if (empty(test) && strcmp(state,"normal"))
    {
      state="back";
    }
    ...
    for (int i = 0; i<3;i++)
    {
      result[i]=branch(qtrs[i])
    }
    if ((result[0]&&result[1]==true) && (result[2]==false))
    {
