...
bool wall_following()
{
  ...
  while (following)
  {
    ...
    nums[0] += std::to_string(front_distance);
    nums[1] += std::to_string(fl_diag_distance);
    nums[2] += std::to_string(left_distance);
    nums[3] += std::to_string(right_distance);
    for (int i = 0; i < 4; ++i)
    {
      Udp.beginPacket("10.17.186.85", 2300);
    ...
    if (front_distance < 12 || fl_diag_distance < 95)
    {
      move(100, -100);
    }
    else
